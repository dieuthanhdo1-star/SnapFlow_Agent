...
const output = new URL(process.env.SNAPFLOW_BROWSER_OUTPUT || '../tmp/folder-watch-20260914/browser/', root);
...
try{
 ...
 await waitFor('document.querySelector("#folder-list [data-folder]")');
 await click('#folder-list [data-folder]');await waitFor('document.querySelector("#folder-input").value.length>0');
 const parentFolder=await evaluate('document.querySelector("#folder-input").value');
 await waitFor('document.querySelector("#folder-list [data-folder]")');await click('#folder-list [data-folder]');
 await waitFor('document.querySelector("#folder-input").value.endsWith("子目录")');
 assert.equal(await evaluate('state.sources.length'),0);
 await click('#folder-up');await waitFor(`document.querySelector('#folder-input').value===${JSON.stringify(parentFolder)}`);
 await screenshot('01b-folder-browser-desktop.png');
 await cdp('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});
 assert.equal(await evaluate('document.querySelector("#consent").scrollWidth<=document.querySelector("#consent").clientWidth'),true);
 await screenshot('01c-folder-browser-mobile.png');
 await cdp('Emulation.setDeviceMetricsOverride',{width:1440,height:1050,deviceScaleFactor:1,mobile:false});
 pass('In-page folder navigation needs no native window and never authorizes while browsing');
 const selectedFolder=await evaluate('document.querySelector("#folder-input").value');
 ...
 await click('#pause-source');await waitFor('state.sources[0].enabled');
 pass('Choose current folder, authorize once, pause and resume use persistent APIs');
 ...
 await click('#settings-open');await waitFor('document.querySelector("#settings").open');assert.equal(await evaluate('document.querySelector("#feishu-callback").textContent'),'http://127.0.0.1:8774/api/feishu/callback');
 ...
 await cdp('Network.enable');await cdp('Network.setBlockedURLs',{urls:['*api/config*']});
 await cdp('Page.reload');await waitFor('document.querySelector("#startup-status").textContent.includes("页面连接失败")');
 await click('#pick-folder');await waitFor('document.querySelector("#consent").open');
 assert.equal(await evaluate('document.querySelector("#folder-input")!==null'),true);
 await cdp('Network.setBlockedURLs',{urls:[]});
 const unexpected=errors.filter(e=>!e.includes('ERR_BLOCKED_BY_CLIENT'));assert.deepEqual(unexpected,[]);
 pass('Folder button still opens panel when initial config request fails');
 await writeFile(new URL('report.json',output),JSON.stringify({passed:results.length,results,errors:errors.filter(e=>!e.includes('ERR_BLOCKED_BY_CLIENT')),expected_fault:'Initial config request blocked to test visible recovery',scope:'Offline browser fixture; native Windows picker and live Feishu not exercised'},null,2));

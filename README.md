...
> **当前状态 · 2026-09-16**<br>
> 文件夹选择、自动监测、分类与本地保存已实现。Windows 默认连接出现 TLS 断开、直连 TCP 超时；已新增连接检测与可选的已有 SSH 转接，并做好一键修复入口。116 项离线检查和 12 组浏览器检查通过；Windows 执行修复与真实截图成功仍待验证，飞书真实账号同步也尚未验收。
>
...
**已有 Windows 安装的下一步**：下载并双击 [`SnapFlow-Repair.cmd`](deliverables/snapflow/SnapFlow-Repair.cmd)。它会备份并更新原安装、检测连接，连接通过后至多重试一张此前失败的截图。无需复制 PowerShell 命令或重填 Gemini 密钥；[校验值](deliverables/snapflow/SnapFlow-Repair.sha256)。
...
| **Gemini 接入** | 默认路由 `gemini-3.8-flash`，已有配置继承与独立保存已实现 | 连接检测、错误状态和可选 SSH 转接已实现；**Windows 真实截图仍待验收** |
...
| **交付与版本管理** | 当前测试包、校验、Git 保存流程和进度文档已建立 | 已新增一个双击修复入口，备份旧代码并保留配置和数据；Windows 执行待确认 |
...
| 优先级 | 工作 | 完成标准 |
| :---: | :--- | :--- |
| **P0** | 定位 Windows 的网关连接失败 | 在 Windows 双击修复入口，确认自动返回诊断，并成功识别一张真实截图 |

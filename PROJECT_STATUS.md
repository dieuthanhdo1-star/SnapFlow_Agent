...
- Windows 在线检查成功，但 SSH 与 WinRM 入口拒绝连接，无法从开发机直接执行修复。`SnapFlow-Repair.cmd` 已于 2026-09-16 09:31:10（开发机日志时间）通过 Tailscale 发送成功；尚未收到 Windows 执行结果，不能写成已安装或已跑通。
...
保持 Windows 和 Tailscale 在线，收到或从仓库下载 `SnapFlow-Repair.cmd` 后双击一次。等待程序自动检测和返回不含图片/密钥的状态；如现有 SSH 身份不可自动登录，继续依据诊断处理，不要求用户反复输入命令。

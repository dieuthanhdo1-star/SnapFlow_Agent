...
2. **已通过（用户确认）**：飞书连接后首条手动同步成功。**当前进行**：启用所选目标的自动同步，放入新截图验证无需点击同步即可在飞书出现，并核对标题、日期与归属。
...
已检查现有应用具备 OAuth、日历选择、我的任务连接和显式自动同步授权。权限 JSON 与当前 OAuth 四项请求一致。接入时需完成后台配置和本人授权；用户现已确认首条手动同步成功，下一步验证自动同步，并补齐任务和日程两种类型的验收。流程依据：[飞书官方登录示例](https://github.com/larksuite/lark-samples/blob/main/web_app_with_auth/python/README.zh.md)。
...
4. 用户已确认首条飞书手动同步成功；任务与日程分别创建、时间、自动同步及幂等性仍待完整验收。
...
在 SnapFlow 的“设置与连接 → 飞书与日程”勾选之后日程自动同步到所选日历，点击“确认同步授权”。再放入一张内容和时间明确、此前未处理的新截图，检查是否无需手动点击即可同步到飞书。任务自动同步需先连接“我的飞书任务”，再单独授权。已有记录不会因开启开关而批量补同步。通过后再验证重复图片和重启恢复。

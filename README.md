...
> **当前状态 · 2026-09-17**<br>
> 文件夹选择、自动监测、分类与本地保存已实现。Windows 默认连接出现 TLS 断开、直连 TCP 超时；已新增连接检测与可选的已有 SSH 转接，并做好一键修复入口。116 项离线检查和 12 组浏览器检查通过；用户已明确确认新截图放进授权文件夹后能自动出现结果，首个自动处理场景通过。用户随后确认“飞书同步上了”，首条手动同步的用户验收通过；用户选择飞书逐条手动确认，本次交付为 Windows 试用版；分类质量、去重及重启恢复仍待验收。
>
...
**交付说明**：[使用与演示指南](docs/WINDOWS_HANDOFF.md) · [离线图文版](docs/WINDOWS_HANDOFF.html)。ZIP 内先打开 `先看这里.html`，再按说明启动；已跑通的电脑继续使用原目录。交付不包含个人配置与账号授权。

**仍遇到连接故障的已有安装**：下载并双击 [`SnapFlow-Repair.cmd`](deliverables/snapflow/SnapFlow-Repair.cmd)。它会备份并更新原安装、检测连接，连接通过后至多重试一张此前失败的截图。无需复制 PowerShell 命令或重填 Gemini 密钥；[校验值](deliverables/snapflow/SnapFlow-Repair.sha256)。

## 产品做什么

用户选择一个截图文件夹并授权，SnapFlow 持续检查新截图和内容发生变化的截图。模型提取信息，按六类能力整理；信息明确的内容保存为记录，缺关键信息的事项交给用户核对。用户核对本地内容后，逐条点击“同步飞书”并确认目标，创建任务和日程。当前交付保持两个自动同步选项关闭。
...
flowchart TD
    ...
    J -->|可处理| K0{连接路径可用}
    K0 -->|检测或修复失败| L
    K0 -->|可用| K[调用 Gemini 3.8 Flash 网关路由]
...
**连接检测**：先检查默认网络，再检查请求级直连；一键修复可启用已有 SSH 路径作为备用。模型列表检测通过不等于识别成功，只有截图响应有效才显示最近识别成功。转接时需要 Tailscale 和开发机在线，TLS 始终验证原网关域名和证书。
...
| **自动监测** | 约 3 秒检查；稳定写入、内容去重、同名修改处理及重启保留授权已实现 | 用户已确认新增截图自动出现结果；多图持续处理、重复内容与重启仍待验收 |
| **Gemini 接入** | 默认路由 `gemini-3.8-flash`，已有配置继承与独立保存已实现 | 用户已确认新截图自动处理可用；分类准确性与长期稳定性仍待验收 |
| **分类与本地记录** | 六类处理、自动保存、待核对、编辑、搜索及批量上传已实现 | 用用户实际截图验证准确性并修正误分类 |
| **飞书与日程** | OAuth、目标选择、任务/日程创建接口及自动同步授权已实现；支持日历文件导出 | 用户确认首条手动同步成功；采用逐条确认，日期时间及去重仍待验收 |
| **调用与预算** | 记录调用状态、模型、部分 token 信息及预算规则 | 未接通可靠人民币费用数据；不能声称已自动执行额度扩充 |
| **交付与版本管理** | 当前测试包、校验、Git 保存流程和进度文档已建立 | 已新增一个双击修复入口，备份旧代码并保留配置和数据；用户已反馈恢复可用，完整升级与重启验收待完成 |

### 验证依据

- **Windows 用户验收反馈**：用户在放入新截图后明确确认“能自动出来”。随后确认“飞书同步上了”，记录首条手动同步成功。尚未分别确认任务与日程两种类型，也不等于自动同步、全部分类或重启验收通过。

- **2026-09-16 网络修复回归：116 项离线检查、12 组 Chrome 交互通过。** 开发机带凭据读取模型列表成功并列出 Gemini 3.8 Flash；转接 TLS 检查通过。均未新增模型调用，不能替代 Windows 实机验收。见 [本次报告](snapflow/verification/network-repair-20260916/REPORT.md)。
...
| **已通过** | 飞书首条手动同步 | 用户确认飞书已显示同步结果；尚未分别验收任务与日程两类 |
| **P0 · 当前** | Windows 试用版交付 | 已跑通自动整理与首条手动同步；保持人工确认，提供干净 ZIP、启动说明和演示步骤 |
...
5. 放入一张测试截图，确认产生有效结果后再增加测试量。用户已反馈修复后可用，仍应以实际截图输出确认；“已配置”或连接检测通过不等于识别成功。
...
│   ├── vision_network.py     连接检测、直连与已有 SSH 转接
